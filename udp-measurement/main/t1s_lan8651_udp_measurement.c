...
/*
 * UDP payload used by the measurement test.
 ...
 * interface as ready for the UDP measurement tasks.
 *
 * @param arg User context passed during event registration. Not used.
...
static void print_rx_results_if_ready(bool force)
{
    ...
    ESP_LOGI(TAG, "========== RX RESULT local_node=%d ==========", CONFIG_APP_NODE_ID);
    ...
    for (int i = 0; i < TEST_MAX_NODES; i++) {
        ...
        ESP_LOGI(TAG,
                 "RESULT_RX local_node=%d src_node=%d packets=%" PRIu32
...
/**
 * @brief Receive UDP measurement packets and collect per-source statistics.
 *
 * The task waits until the Ethernet interface has an IP address, binds a UDP
 * socket to the configured test port, validates incoming measurement headers,
 * and updates packet, byte, sequence, loss, and timing counters for each source
 * node.
 *
 * @param arg FreeRTOS task argument. Not used.
 */
static void measurement_rx_task(void *arg)
{
    (void)arg;
...
/**
 * @brief Transmit UDP measurement packets from the configured sender node.
 *
 * Nodes whose APP_NODE_ID does not match APP_TEST_SENDER_ID terminate this
 * task and operate as receivers only. The sender waits for the configured
 * start delay, transmits UDP broadcast packets for the configured duration,
 * and prints final transmit counters and throughput values.
 *
 * @param arg FreeRTOS task argument. Not used.
 */
static void measurement_tx_task(void *arg)
{
    (void)arg;
    ...
    if (CONFIG_APP_NODE_ID != CONFIG_APP_TEST_SENDER_ID) {
        ESP_LOGI(TAG, "This node is receiver only. Sender node is %d",
                 CONFIG_APP_TEST_SENDER_ID);
        vTaskDelete(NULL);
        return;
    }

    ESP_LOGI(TAG, "This node is measurement sender");
    ...
    dest_addr.sin_addr.s_addr = inet_addr(TEST_BROADCAST_IP);
    ...
    ESP_LOGI(TAG,
             "Starting UDP measurement test: sender=%d duration=%d s payload=%d B tx_delay=%d us",
             CONFIG_APP_NODE_ID,
    ...
    ESP_LOGI(TAG, "========== TX RESULT node=%d ==========", CONFIG_APP_NODE_ID);
    ESP_LOGI(TAG,
             "RESULT_TX node=%d packets=%" PRIu64 " errors=%" PRIu32
             " udp_bytes=%" PRIu64 " payload_bytes=%" PRIu64
             " duration_s=%.3f udp_mbps=%.3f payload_mbps=%.3f",
             CONFIG_APP_NODE_ID,
             tx_packets,
...
void app_main(void)
{
    ...
    ESP_LOGI(TAG, "Test sender ID: %d", CONFIG_APP_TEST_SENDER_ID);
